...
{
    //Take text file in and read it into a string using the input stream buffer iterator
    std::ifstream input_file(filename);
    std::string file_text((std::istreambuf_iterator<char>(input_file)), std::istreambuf_iterator<char>());
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
    ...
    //Getting current time and using tm struct to get day, month and year
    time_t date = time(0);
    ...
    ofile << "Time: "<< 1900 + local_time.tm_year << "-" << 1 + local_time.tm_mon << "-" << local_time.tm_mday << std::endl;
    ofile << "Key: " << key << std::endl;
    ofile << data << std::endl;
    ofile.close();
}

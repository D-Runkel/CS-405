...
{
    /*
    * Well, this isn't exactly how I would go about defending a SQL injection attack, but I understand the intention here.
    * 
    * To detect a sql injection attack with only the raw query, I am limited to examining the query for attack patterns,
    * specifically the injection of the typical ( or []=[]) which is a common attack vector, and utilized in this code
    * 
    * Seeing as parameterization and prepared statements is off the table, I decided to examine our query for instances of ( or [something] = [something]),
    * as the query we are given does not utilize that pattern.
    */
    std::regex regexp(" or .*=.*");

    if (std::regex_search(sql, regexp))
    {
        std::cout << "SQL injection attack is detected. User attempted to use invalid character patterns" << std::endl;
        return false;
    }
